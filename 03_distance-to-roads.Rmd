...
```{r plot_histogram, eval=FALSE, echo=FALSE}
...
```{r show_dist_roads_nnb, eval=TRUE, message=FALSE, echo=FALSE}
# read in and show
library(magrittr)
...
## Histograms of distance to nearest site

```{r get_nn_site, eval=FALSE}
...
```{r plot_nn_site_sp, eval=FALSE, echo=FALSE}
...
## Table: Nearest site where the species was seen
...
```{r show_sp_nnb, eval=TRUE, message=FALSE, echo=FALSE}
# show table of distance to nearest site for each species
readr::read_csv("data/results/dist_nnb_species_specific.csv") %>%
  knitr::kable()
```

**Is this figure necessary here?**

![Histograms showing the species-specific distances to nearest neighbouring site.](figs/fig_nnb_species.png)

## Visualise distance to roads and nearest site
...
```{r plot_map_dist_roads, eval=FALSE, echo=FALSE}
# plot on maps
ggplot() +
...
```{r plot_map_nnb, eval=FALSE, echo=FALSE}
...
![Distribution of checklist locations across the Nilgiri, Anamalai and Palani Hills. Inset histogram shows the distribution of checklists' distance to the nearest road, with the X-axis on a log-scale.](figs/fig_distRoads.png)
