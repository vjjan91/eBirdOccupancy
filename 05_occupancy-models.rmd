...
dat <- fread("data/dataCovars_10km.csv",header=T)
setDF(dat)
head(dat)
...
dat <- dat %>% 
  mutate(effort_distance_km = replace(effort_distance_km, 
...
# Rename column names:
names(dat) <- c("duration_minutes", "effort_distance_km","locality", 
...
fwrite(dat.scaled, file = "data/scaledCovs_10km.csv")

dat.scaled <- fread("data/05_scaled-covars-10km.csv",header=T)
setDF(dat.scaled)
head(dat.scaled)

# Ensure observation_date column is in the right format
dat.scaled$observation_date <- format(as.Date(dat.scaled$observation_date,
                                              "%m/%d/%Y"),
                                      '%Y-%m-%d')
...
for(i in 1:length(unique(dat.scaled$scientific_name))){
  ...
  occ_gof <- mb.gof.test(model_lc_clim, nsim = 1000, plot.hist = FALSE)
...
write.csv(goodness_of_fit, "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\output\\05_goodness-of-fit-10km.csv")
