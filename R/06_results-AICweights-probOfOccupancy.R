...
model_imp <-group_by(model_imp, predictor) %>%
  summarise(mean_AIC = mean(AIC_weight),
...
predictor_name <- tibble(predictor = levels(model_imp$predictor),
                         pred_name = c("Annual Mean Temperature (°C)", 
...
fig_aic <-
  ggplot(model_imp)+
  geom_pointrange(aes(x = reorder(predictor, mean_AIC),
                      y = mean_AIC,
...
fig_predictor <-
  ...
  geom_col(data = data_predictor_long,
           aes(x = reorder(predictor, mean_AIC),
               y = magnitude,
               fill = effect),
