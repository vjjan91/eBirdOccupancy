...
gridsize = 250 # grid size in metres

# split data by species
dataSpatialThin <- split(x = dataGrouped, f = dataGrouped$scientific_name)

# spatial thinning on each species retains
# site with maximum visits per grid cell
coordsUTM <- map(dataSpatialThin, function(df){
  df %>% 
    ...
    st_coordinates(.) %>% 
    as_tibble() %>% 
    mutate(X = plyr::round_any(X, gridsize),
           Y = plyr::round_any(Y, gridsize))
})

# add rounded coordinates to data
dataSpatialThin <- map2(dataSpatialThin, coordsUTM, function(df1, df2){
  df1 <- bind_cols(df1, df2)
  
  # count visits
  n_visits <- count(df1, longitude, latitude, name = "n_visits")
  
  # bind visits to coordinate data
  df1 <- left_join(df1, n_visits, by = c("longitude", "latitude"))
  
  # select max site per rounded UTM coords X and Y
  df1 <- group_by(df1, X, Y) %>% 
...
dataSubsample <- dataSpatialThin %>% 
  map(function(df){
    locality_list <- split(x = df, f = df$locality)
    locality_list <- map_if(.x = locality_list,
...
data_at_scale$data <- map(data_at_scale$data, function(df){
  ...
  df <- inner_join(dataSubsample, df, by=c("longitude", "latitude"))
  return(df)
})
```
