...
### Apply explicit spatial filter

```{r spatial_subset_main, eval=FALSE}
# get hills and subset
wg <- st_read("data/spatial/hillsShapefile/Nil_Ana_Pal.shp")
ebd_locs <- dataGrouped %>% 
...
dataSubsample <- dataGrouped %>%
  filter(longitude %in% ebd_locs$X, latitude %in% ebd_locs$Y) %>% 
  # sample_n(1e3) %>% # SAMPLING 1000 AS AN EXAMPLE
  
  plyr::dlply(c("scientific_name", "locality_id")) %>% # ugh this needs replaced because dlply is deprecated
