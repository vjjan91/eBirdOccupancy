...
library(lubridate)
jul.date <- as.Date(dataCovar$observation_date)
dataCovar$jul.date <- yday(jul.date)

#### mean time, distance and number of observers per locality and totalVisits to each localtion
...
# Obtaining a dataframe for occupancy modeling
dataOccu <-  dataSubsample %>% group_by(group = sampling_event_identifier) %>% spread(group,pres_abs)
...
## Having issues with the line below, not able to write it
write_csv(dataOccuFin,"E:\\Occupancy Modeling\\Scripts\\Occupancy Scripts\\DataForOccupancyMod.csv")
