...
### Leaving comments here for Pratik:
1. Need to get multiple dataframes for each variable so that they can be called in separately using the occupancy code.
2. I need to get a dataframe that matches the locality ID for the pres_abs dataframe you created above, with the 
locality id for the DataCovar that contains all covariates. Basically, I need to run the EDA that will be done below for the 5825 unique localities only. Does that make sense? 
