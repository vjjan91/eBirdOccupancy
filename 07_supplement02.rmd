...
ebd_summary <- ebd_summary[,.(nrep = length(unique(sampling_event_identifier))), by = .(X,Y,nchk,scientific_name)]
...
ebd_summary <- setDF(ebd_summary) %>% 
  complete(nesting(X,Y), scientific_name, fill = list(p_rep = 0))
```
