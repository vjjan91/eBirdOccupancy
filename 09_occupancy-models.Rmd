...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  ...
  model_lc_clim <- occu(~ min_obs_started +
    ...
    effort_distance_km +
    number_observers +
    expertise ~  lc_01 + lc_02 + lc_05 + lc_04 + lc_09 + lc_07 + lc_03 + bio4a + I(bio4a^2)+ bio15a + I(bio15a^2), data = occ_um)
  ...
  lc_clim[[i]] <- pdredge(model_lc_clim, clust, fixed = det_terms,
                          subset = (dc("psi(bio4a)","psi(I(bio4a^2))") &
                                    dc("psi(I(bio4a^2))", "psi(bio4a)") &
                                    dc("psi(bio15a)","psi(I(bio15a^2))") &
                                    dc("psi(I(bio15a^2))","psi(bio15a)")))
...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  dat.scaled %>% filter(dat.scaled$scientific_name == unique(dat.scaled$scientific_name)[i])
  ...
  model_lc_clim <- occu(~ min_obs_started +
    ...
    lc_04 + lc_09 + lc_07 + lc_03 +  bio4a + I(bio4a^2)+ bio15a + I(bio15a^2), data = occ_um)
  
  occ_gof <- mb.gof.test(model_lc_clim, nsim = 100, parallel = T, ncores = 5, 
                         plot.hist = FALSE)
