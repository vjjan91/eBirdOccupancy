...
cols_to_scale <- c(
  ...
  "number_observers", "expertise", "elev", "bio_1", "bio_12", "lc_02", "lc_09", "lc_01", "lc_05", "lc_04", "lc_07", "lc_03", "julian_date", "min_obs_started"
)
...
for (i in species) {
  ...
  occ_wide <- format_unmarked_occu(occ,
    ...
    site_covs = c(
      ...
      "lc_04", "lc_09", "lc_07", "lc_03", "bio_1", "bio_12"
    ),
    obs_covs = obs_covs
  )
...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  ...
  model_lc_clim <- occu(~ min_obs_started +
    ...
    expertise ~ lc_01 + lc_02 + lc_05 + lc_04 + lc_09 + lc_07 + lc_03 + 
      bio_1 + I(bio_1^2) + bio_12 + I(bio_12^2), data = occ_um)
  ...
  lc_clim[[i]] <- pdredge(model_lc_clim, clust,
    fixed = det_terms,
    subset = (dc("psi(bio_1)", "psi(I(bio_1^2))") &
      dc("psi(I(bio_1^2))", "psi(bio_1)") &
      dc("psi(bio_12)", "psi(I(bio_12^2))") &
      dc("psi(I(bio_12^2))", "psi(bio_12)"))
  )
...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  ...
  model_lc_clim <- occu(~ min_obs_started +
    ...
    lc_04 + lc_09 + lc_07 + lc_03 + bio_1 + I(bio_1^2) + bio_12 + I(bio_12^2), data = occ_um)

  occ_gof <- mb.gof.test(model_lc_clim,
    nsim = 100, parallel = T, ncores = 5,
    plot.hist = FALSE
  )
...
write.csv(goodness_of_fit, "data/results/goodness-of-fit-2.5km.csv")
```
