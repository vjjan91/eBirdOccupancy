...
Read the data for our dry season months; Jan -- May, and December.
...
### Prepare Bioclim 1 (mean temperature) for dry season

```{r check_for_4a}
if (file.exists("data/chelsa/CHELSA_bio10_1.tif")) {
  message("Bio 1 already exists, will be overwritten")
} else {
  message("Bio 1 does not exist, will be prepared")
}
```
...
Here, we use only the months of December and Jan -- May for dry season temperature.
...
# calculate Bio 1 for dry season
bio_1 <- (calc(stack(tkAvg), fun = mean)
names(bio_1) <- "CHELSA_bio10_1"

# save bio_1
writeRaster(bio_1, filename = "data/chelsa/CHELSA_bio10_1.tif", overwrite = T)
```

### Prepare Bioclim 12 (total rainfall) for dry season

```{r check_for_15}
if (file.exists("data/chelsa/CHELSA_bio10_15.tif")) {
  message("Bio 12 already exists, will be overwritten")
} else {
  message("Bio 12 does not exist, will be prepared")
}
```

Bioclim 12, the total precipitation (in our area, rainfall), and equals

where $\Sigma PPT_i$ is the monthly precipitation for the dry season months of December and Jan -- May.
...
# calculate Bio 4a
bio_12 <- (calc(stack(pptTotal), fun = sum))
names(bio_12) <- "CHELSA_bio10_12"

# save bio_4a
writeRaster(bio_12, filename = "data/chelsa/CHELSA_bio10_12.tif", overwrite = T)
```
...
env_data <- stack(elevData, bio_1, bio_12)
```
...
![**Climate and landcover vary strongly along the elevation gradient in the Nilgiri and Anamalai Hills.**
Both (a) variation in temperature and (b) variation in rainfall, between the months of December and May, declines with increasing elevation across the Nilgiri and Anamalai Hills. Climatic variation is not very strongly associated with land cover type, as both natural habitats such as forests, and artificial habitats such as plantations show low variation in (c) temperature, and (d) rainfall. (e) Most elevations host a range of land cover types: while artificial types such as agriculture are concentrated at lower elevations, and more natural types such as grasslands and forests are associated with higher elevations, each of these types is also found outside their characteristic elevational bands. We calculated climate seasonalities (BIOCLIM 4a and 15: temperature and rainfall, respectively) using CHELSA data over 1979 – 2013, from December to May (Karger et al. 2017), and present mean seasonality values (vertical bars show standard deviation) per 200 m elevational band. Land cover types were taken from a reclassification of Roy et al. (2015; see main text) at 100 m elevational bands. Land cover types covering < 1% of an elevational band are shaded grey. All landscape layers were first resampled to 1 km resolution.](figs/fig_02.png)
