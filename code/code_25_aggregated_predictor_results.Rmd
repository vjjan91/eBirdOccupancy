...
fig_predictor_effect <-
  ...
  geom_col(aes(x = factor(predictor), y = negative,
               group = species_distribution,
...
data_predictor_effect <- mutate(data_predictor_effect, 
                       direction = coefficient > 0) %>% 
  count(scale, predictor,`Habitat (Open Country / Forest )`, direction) %>% 
  mutate(mag = n * (if_else(direction, 1, -1)))
...
data_predictor_effect <- mutate(data_predictor_effect, 
                       direction = coefficient > 0) %>% 
  count(scale, predictor,`Foraging Space (Canopy / Mid-storey / Terrestrial / Shrub)`, direction) %>% 
  mutate(mag = n * (if_else(direction, 1, -1)))
