...
**CHECKED UNTIL HERE** --- REMOVE THESE AND OTHER NOTES LATER
...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  data <- dat.scaled %>%
    filter(dat.scaled$scientific_name == unique(dat.scaled$scientific_name)[3])
  ...
  occ_wide <- format_unmarked_occu(occ,
    ...
    response = "pres_abs",
    site_covs = c(
     "locality_id", "bio4a.y","lc_01.y", "lc_02.y", "lc_05.y",
       "lc_04.y", "lc_07.y","lc_09.y","lc_03.y", "bio15a.y"
    ),
    obs_covs = obs_covs
  )
