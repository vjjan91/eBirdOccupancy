...
fig_predictor_effect <- patchwork::wrap_plots(data_predictor_long$figs,
                                              nrow = 1)
# save plot
ggsave(fig_predictor_effect, filename = "figs/fig_lc_v_clim.png",
       dpi = 300)
```
...
data_lc_v_clim <- data_predictor_long %>% 
  mutate(predictor = if_else(str_detect(predictor, "bio"), 
                             "climate", "landcover")) %>% 
  group_by(trait, scale, predictor, value) %>%
  summarise_at(.vars = c("negative", "positive"),
               .funs = list(sum))
```

```{r}
# pivot for plot, groups maintained
data_lc_v_clim <- data_lc_v_clim %>% 
  pivot_longer(cols = c("negative", "positive"),
               names_to = "effect",
               values_to = "magnitude")
...
data_lc_v_clim$figs <- map(data_lc_v_clim$cols,
                           function(df) {
                             ggplot(df) +
