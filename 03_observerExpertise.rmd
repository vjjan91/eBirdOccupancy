...
library(sf)
wg <- st_read("C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\Shapefiles\\Nil_Ana_Pal.shp") %>% 
  st_transform(32643)

# set file paths for auk functions
f_in_ebd <- file.path("C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\eBirdDataWG_filtered.txt")
f_in_sampling <- file.path("C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\eBirdSamplingDataWG_filtered.txt")
...
f_out_ebd <- "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\ebird_for_expertise.txt"
f_out_sampling <- "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\ebird_sampling_for_expertise.txt"

ebd_filtered <- auk_filter(ebd_filters, file = f_out_ebd,
                           file_sampling = f_out_sampling, overwrite = TRUE)
```

```{r read_all_ebd, eval=FALSE}
...
ebd <- setDF(ebd) %>% 
  as_tibble() %>% 
  select(one_of(columnsOfInterest))
...
specieslist = read_excel(path = "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\3_List_of_species.xlsx")
...
fwrite(ebdSpSum, file = "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\dataChecklistSpecies.csv")
```
...
library(raster)
landcover <- raster::raster("C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\landcover\\lc_01km.tif")
...
setnames(ebdChkSummary, c("sei", "observer","year", "duration", "distance", "area",
                          "longitude", "latitude", "locality", "locality_id",  "decimalTime",
...
fwrite(ebdChkSummary, file = "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\eBirdChecklistVars.csv")
```
...
# make dir if absent
if(!dir.exists("C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\modOutput")){
  dir.create("C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\modOutput")
}

# write model output to text file
{
  writeLines(R.utils::captureOutput(list(Sys.time(), summary(modObsExp))), 
             con = "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\modOutput\\modOutExpertise.txt")
}
```
...
fwrite(dfPredict %>% dplyr::select(observer, score), file = "C:\\Users\\vr235\\Desktop\\Occupancy Runs\\Data\\dataObsExpScore.csv")
```
