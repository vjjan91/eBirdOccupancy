...
data <- left_join(data, scores, by = c("observer_id" = "observer"))
data <- select(data, score, nSp, totSoiSeen, landcover, year) %>% 
  filter(!is.na(score))
```
...
data_summary01 <- data %>% 
  ...
  select(score, year, nSp, totSoiSeen) %>% 
  pivot_longer(cols = c("nSp", "totSoiSeen"),
