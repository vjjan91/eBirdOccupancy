...
# select species
soi <- c(
  "Iole indica", "Ficedula nigrorufa",
  "Pycnonotus cafer", "Pycnonotus jocosus"
)
...
data_fig <- data_poccu %>%
  ...
  mutate(
    cat = case_when(
      scientific_name %in% c("Iole indica", "Ficedula nigrorufa") ~ "forest",
      T ~ "general"
    )
  )
...
```{r}
# get ranges
range_1 <- terra::minmax(bio_1)[, 1] # divide temp by 10
...
names(climate) = c(
  "CHELSA bio_1 mean annnual temp.", 
  "CHELSA bio_12 total ppt."
)
...
## NO LONGER USED --- WIP --- TO BE REMOVED
