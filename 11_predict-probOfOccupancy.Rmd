...
```{r}
# scale predictors
scale15 <- c(30, 50) # range of precpitation
...
# select species
soi <- c(
  "Myophonus horsfieldii", "Merops leschenaulti",
  "Acrocephalus dumetorum", "Pycnonotus cafer"
)
...
data_fig <- data_poccu %>%
  ...
  mutate(
    cat = case_when(
      scientific_name %in% c("Myophonus horsfieldii", "Merops leschenaulti") ~ "forest",
      T ~ "general"
    )
  )
...
Predicted probability of occupancy curves as a function of temperature seasonality for two (a) generalist and (b) forest species are shown here. (a) Temperature seasonality is negatively associated with the probability of occupancy of the chestnut-headed bee-eater _Merops leschenaulti_ and the Malabar whistling-thrush _Myophonus horsfieldii_. (b) Temperature seasonality is positively associated with the probability of occupancy of the Blyth’s reed warbler _Acrocephalus dumetorum_ and the red-vented bulbul _Pycnonotus cafer_. ](figs/fig_05.png)
...
names(soi_occu) <- c("Myophonus horsfieldii", "Merops leschenaulti","Acrocephalus dumetorum","Pycnonotus cafer")
```
