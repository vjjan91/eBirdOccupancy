...
predictors <- c("bio_\\d+", glue("lc_0{seq(9)}"))
...
```{r}
# scale predictors
scale_12 <- c(200, 1000) # range of precpitation
scale_1 <- c(20, 40) # range of temperatures

# scale bio4a and bio15a by actual values
data_resp <- mutate(
  data_resp,
  pred_val = case_when(
    predictor == "bio_1" ~ scales::rescale(pred_val, to = scale_1),
    predictor == "bio_12" ~ scales::rescale(pred_val, to = scale_12),
    T ~ pred_val
  )
)
...
# select species
soi <- c(
  "Pomatorhinus horsfieldii", "Ficedula nigrorufa",
  "Pycnonotus cafer", "Psittacula krameri"
)
which_predictors <- c("bio_1")
```
...
data_fig <- data_poccu %>%
  ...
  mutate(
    cat = case_when(
      scientific_name %in% c("Pomatorhinus horsfieldii", "Ficedula nigrorufa") ~ "forest",
      T ~ "general"
    )
  )
...
![**Probability of occupancy as a function of mean annual temperature.**
Predicted probability of occupancy curves as a function of mean annual temperature (in celsius) for two (a) generalist and (b) forest species are shown here. (a) Mean annual temperature is positively associated with the rose-ringed parakeet _Psittacula krameri_ and the red-vented bulbul _Pycnonotus cafer_. (b) Mean annual temperature is negatively associated with the black-and-orange flycatcher _Ficedula nigrorufa_ and the indian scimitar-babbler _Pomatorhinus horsfieldii_.](figs/fig_05.png)
...
bio_1 = lscape[[4]] / 10 # temp, divide by 10
bio_12 = lscape[[5]] # rain
...
```{r}
# get ranges
range_1 <- terra::minmax(bio_1)[, 1] 
range_12 <- terra::minmax(bio_12)[, 1]

# rescale
bio_1 <- (bio_1 - min(range_1)) / (diff(range_1))
bio_12 <- (bio_12 - min(range_12)) / (diff(range_12))

# project to UTM
climate <- c(bio_1, bio_12)
names(climate) = c(
  "Mean annual temp.", 
  "Annual precip."
)
...
names(soi_occu) <- c("Ficedula nigrorufa","Pomatorhinus horsfieldii","Pycnonotus cafer","Psittacula krameri")
```
...
![**Predicted area of occurrence as a function of mean annual temperature.**
Predicted area of occurrence as a function of mean annual temperature (in celsius) for two forest and two generalist species are shown here. The endemic and range-restricted black-and-orange flycatcher _Ficedula nigrorufa_ has a higher probability of occupancy at high elevations across the Nilgiris and the Anamalai hills while the Indian scimitar-babbler _Pomatorhinus horsfieldii_ is predicted to occur at higher probabilities at mid and high elevations. The red-vented bulbul _Pycnonotus cafer_ and the rose-ringed parakeet _Psittacula krameri_, both generalist species, are widely distributed at low elevations and have reduced probability of occurrence at mid and high elevations across our study area. ](figs/fig_06.png)
