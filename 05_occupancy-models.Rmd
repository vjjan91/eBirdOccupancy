...
names(dat)
dat <- dat[, -c(1, 4, 5, 16, 18, 21, 23, 25, 26, 37)]
...
names(dat) <- c(
  ...
  "lc_07.y","lc_03.y","min_obs_started", "julian_date"
)
...
dat.1 <- dat %>%
  mutate(
    ...
    min_obs_startedSq = (min_obs_started)^2,
    julian_dateSq = (julian_date)^2
  ) # occupancy modeling requires an integer response

# Scaling detection and occupancy covariates
dat.scaled <- dat.1
dat.scaled[, c(1, 2, 11, 16:28, 30,31)] <- scale(dat.scaled[, c(1, 2, 11, 16:28,30,31)]) # Scaling and standardizing detection and site-level covariates
...
source("R/fun_screen_cor.R")
names(dat.scaled)
screen.cor(dat.scaled[, c(1, 2, 11, 16:28,30,31)], threshold = 0.3)
...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  ...
  global_mod <- occu(~ min_obs_started + min_obs_startedSq +
    julian_date + julian_dateSq +
    duration_minutes +
    effort_distance_km +
    number_observers +
...
for (i in 7:length(unique(dat.scaled$scientific_name))) {
  ...
  model_lc_clim <- occu(~ min_obs_started + min_obs_startedSq + 
    julian_date + julian_dateSq + 
    duration_minutes +
    effort_distance_km +
    number_observers +
