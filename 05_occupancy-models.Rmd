...
dat <- dat %>%
  mutate(newjulianDate = 
  case_when(julian_date >=334 & julian_date <=365 ~ (julian_date - 333),
            julian_date >=1 & julian_date <=152 ~ (julian_date + 31)
            )) %>%
  drop_na(newjulianDate)

# recode time observations started to model it as a linear predictor
dat <- dat %>%
  mutate(newmin_obs_started = case_when(min_obs_started >=300 & min_obs_started <=720 ~ abs(min_obs_started - 720),
min_obs_started >=720 & min_obs_started <=1140 ~ abs(720- min_obs_started))) %>%
...
dat.scaled <- dat.1
dat.scaled[, c(1, 2, 11, 16:28)] <- scale(dat.scaled[, c(1, 2, 11, 16:28)]) # Scaling and standardizing detection and site-level covariates
...
for (i in 1:length(unique(dat.scaled$scientific_name))) {
  ...
  occ_wide <- format_unmarked_occu(occ,
    site_id = "site",
    response = "pres_abs",
    site_covs = c("locality_id", "lc_01.y", "lc_02.y", "lc_05.y", 
...
write.xlsx(det_dred, file = "C:\\Users\\vr235\\Downloads\\det-dred.xlsx")

# 2. Store all the model averaged outputs for each species and the relative importance score
write.xlsx(det_avg, file = "C:\\Users\\vr235\\Downloads\\det-avg.xlsx", rowNames = T, colNames = T)
write.xlsx(det_imp, file = "C:\\Users\\vr235\\Downloads\\det-imp.xlsx", rowNames = T, colNames = T)

write.xlsx(det_modelEst, file = "C:\\Users\\vr235\\Downloads\\det-modelEst.xlsx", rowNames = T, colNames = T)
...
write.xlsx(lc_clim, file = "C:\\Users\\vr235\\Downloads\\lc-clim.xlsx")

# 2. Store all the model averaged outputs for each species and relative importance scores
write.xlsx(lc_clim_avg, file = "C:\\Users\\vr235\\Downloads\\lc-clim-avg.xlsx", rowNames = T, colNames = T)
write.xlsx(lc_clim_imp, file = "C:\\Users\\vr235\\Downloads\\lc-clim-imp.xlsx", rowNames = T, colNames = T)

# 3. Store all model estimates
write.xlsx(lc_clim_modelEst, file = "C:\\Users\\vr235\\Downloads\\lc-clim-modelEst.xlsx", rowNames = T, colNames = T)
```
