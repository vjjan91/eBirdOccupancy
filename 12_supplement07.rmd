...
# load checklist data and select one per rounded 500m coordinates
{
  ...
  scale <- c(100, 500, 1000, 2500, 5000)
...
# round to scale
data <- mutate(data, data = map2(scale, data, function(scale, df){
  df <- df %>% 
    ...
    st_drop_geometry() %>% 
    mutate(X_round = plyr::round_any(X, scale),
           Y_round = plyr::round_any(Y, scale)) %>% 
    
    group_by(X_round, Y_round) 
  
  # what's the max point in each grid
  max <- filter(df, tot_effort == max(tot_effort)) %>% 
    ungroup() %>% 
    summarise(prop_points= length(X)/n_all_points,
              prop_effort = sum(tot_effort)/d_all_effort) %>% 
    pivot_longer(cols = everything(),
                 names_to = "variable")
  
  # which are the points kept
  points_max <- filter(df, tot_effort == max(tot_effort)) %>% 
    ungroup() %>% 
    select(-contains("round"))
  
  # select a random point in each grid
  rand_sites <- # bind_rows(rand_sites) %>% 
    group_by(df, X_round, Y_round) %>% 
    sample_n(1) %>% 
    # summarise(mean_tot_effort = mean(tot_effort)) %>% 
    ungroup() %>% 
    summarise(prop_points = length(X_round)/n_all_points,
              prop_effort = sum(tot_effort)/d_all_effort) %>% 
    pivot_longer(cols = everything(),
                 names_to = "variable")
  
  # which random points are kept
  points_rand <- sample_n(df, size = 1)
  
  df <- tibble(grid_rand = list(rand_sites), grid_max = list(max),
...
```{python prep_py_libs_supp07, eval=FALSE}
...
```{python make_and_write_networks}
...
```{r continue_network_thinning, eval=FALSE}
...
```{r remove_network_thinned_sites, eval=FALSE}
# get python sites
ul = py$ul
...
```{r compare_points_remaining, eval=FALSE}
...
# get scales as list
scales_list <- list(100,500,1000, rep(scale, 2)) %>% flatten()
...
```{python, process_for_spat_indep, eval=FALSE}
...
```{r compare_methods_plot, eval=FALSE}
...
# plot results
fig_spatial_thinning <- ggplot(data_plot)+
...
# save
ggsave(fig_spatial_thinning, filename = "figs/fig_spatial_thinning.png", width = 10, height = 4,
       dpi=300); dev.off()
```
